...
const callShopify = async query => {
  const fetchUrl = `https://${domain}/api/2022-07/graphql.json`
  const fetchOptions = {
    endpoint: fetchUrl,
    method: 'POST',
    headers: {
...
export const getPaginatedCollection = async (handle, cursorNext) => {
  const query = paginatedCollection(handle, cursorNext)
  const response = await callShopify(query)
  const productsByCollection =
		response.data.collection && response.data.collection
...
export const getPaginatedCollectionBack = async (handle, cursorPrev) => {
  const query = paginatedCollectionBack(handle, cursorPrev)
  const response = await callShopify(query)
  console.log(`%c res`, 'background: #0047ab; color: #fff; padding: 2px:', response)
  const productsByCollection =
		response.data.collection && response.data.collection
